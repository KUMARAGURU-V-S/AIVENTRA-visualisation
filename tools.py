...
def _serialize_chart(fig, serialize: bool):
    """Return Plotly figure as JSON string if serialize=True, else return figure object."""
    if serialize and fig is not None:
        return fig.to_json()
    return fig


def detect_high_frequency_callers(df: pd.DataFrame, threshold_percentile: float = 95, serialize: bool = False) -> dict:
    ...
    return {
        ...
        "chart": _serialize_chart(fig, serialize),
    }


def analyze_time_distribution(df: pd.DataFrame, serialize: bool = False) -> dict:
...
def detect_anomalous_charges(df: pd.DataFrame, z_threshold: float = 2.5, serialize: bool = False) -> dict:
...
def find_suspicious_international(df: pd.DataFrame, intl_call_threshold: int = 8, serialize: bool = False) -> dict:
...
def cluster_behavior_patterns(df: pd.DataFrame, n_clusters: int = 5, serialize: bool = False) -> dict:
...
def detect_burner_phones(df: pd.DataFrame, max_account_length: int = 40, serialize: bool = False) -> dict:
...
def correlation_analysis(df: pd.DataFrame, serialize: bool = False) -> dict:
...
def generate_suspect_timeline(df: pd.DataFrame, phone_number: str = None, serialize: bool = False) -> dict:
